...
class Network:
    ...
    def update(self, tau=1e-4):
        eval_params = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope="eval")
    ...
    def __init__(self, sess, state_dim, action_dim, learning_rate):
        ...
        self.double_target = tf.reduce_sum(
            tf.multiply(
                self.target, tf.one_hot(tf.argmax(self.eval_ns, axis=-1), self.a_dim)
            ),
            reduction_indices=1,
            keepdims=True,
        )
        ...
        self.target_params = tf.get_collection(
            tf.GraphKeys.TRAINABLE_VARIABLES, scope="target"
        )
        self.soft_update = [
        ...
        self.loss = tflearn.mean_square(
            tf.reduce_sum(
                tf.multiply(self.eval, self.acts), reduction_indices=1, keepdims=True
            ),
            self.R,
        )
    ...
    def predict(self, input):
        action = self.sess.run(self.eval, feed_dict={self.inputs: input})
        return action[0]
